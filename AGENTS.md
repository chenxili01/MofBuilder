...
This repository branch uses a **two-agent Codex workflow** to develop the `role-runtime-contract` branch in controlled phases.
...
* snapshot-first API design before optimizer rewrite
...
Optimizer may later consume a snapshot, but does not become the owner of role semantics.
...
* validation goals
* exit criteria
* rollback risks or blockers if relevant
...
6. Never let optimizer consume arbitrary builder internals directly.
7. Never remove the old optimizer path in this branch.
