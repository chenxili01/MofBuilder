...
class OpenmmSetup:
    ...
    def run_em(self, output_prefix='em', whole_traj_file=None):
        ...
        if self.simulation is None:
            integrator = LangevinIntegrator(self.temperature, 1 / picosecond,
                                            self.timestep)
            self.simulation = Simulation(self.top.topology, self.system,
                                         integrator)
        ...
        self.simulation.reporters.append(
            StateDataReporter(f"{output_prefix}.log",
                              1,
                              step=True,
        ...
        PDBFile.writeFile(self.simulation.topology, self.positions,
                          open(em_file, 'w'))
        ...
        if whole_traj_file:
            PDBFile.writeFile(self.simulation.topology, self.positions,
                              open(whole_traj_file, 'a'))

    def _run_md(self,
                mode='nvt',
                time_ps=100,
                output_prefix='traj',
                record_interval_ps=1,
                whole_traj_file=None):
    ...
    def run_pipeline(self,
        ...
        for step in steps:
            if step.lower() == 'em':
                self.run_em(output_prefix=f"{output_prefix}_em",
                            whole_traj_file=whole_traj_file)
            elif step.lower() == 'nvt':
                self.run_nvt(nvt_time=nvt_time,
                             output_prefix=f"{output_prefix}_nvt",
                             record_interval=record_interval,
                             whole_traj_file=whole_traj_file)
            elif step.lower() == 'npt':
                self.run_npt(npt_time=npt_time,
